        ...
        return Component.extend({
            ...
            afterPlaceOrder: function () {
                $.post(url.build('liqpay/checkout/form'), {
                    'random_string': 'Success'//this._generateRandomString(30)
                }).done(function(data) {
                    if (!data.status) {
                        return
                    }
                    if (data.status == 'success') {
                        if (data.content) {
                            var html = '<div id="liqPaySubmitFrom" style="display: block;">' + data.content + '</div>';
                            $('div.payment-method._active').append(html);
